...
export default async function ConstructorDetailPage({ params }: { params: { id: string } }) {
  const constructorData: OrderConstructor | null = await getConstructorById(params.id);
      ...
      <main className="flex-1 py-10">
        <div className="container mx-auto max-w-5xl px-4">
          ...
          <div className="mb-8 rounded-lg border bg-secondary/50 p-6">
            ...
            <p className="mt-2 text-sm text-muted-foreground">企業の基本情報・対応工種・業務範囲・エリアを確認できます。</p>
          </div>
          ...
          <div className="space-y-6">
                  ...
                  <div className="flex-1 space-y-4 text-center md:text-left">
                    <div>
                      <div className="mb-2 flex flex-col items-center gap-2 md:flex-row">
                        <h1 className="text-3xl font-bold">{constructorData.companyName}</h1>
                      ...
                      {constructorData.companyRepresentativeName && (
                        <p className="text-lg text-muted-foreground">代表者: {constructorData.companyRepresentativeName}</p>
                      )}
                    </div>

                    <p className="flex items-center justify-center text-muted-foreground md:justify-start">
                      <MapPin className="mr-2 h-4 w-4" />
                      {constructorData.companyPrefecture} {constructorData.companyCity} {constructorData.companyAddress}
                    </p>
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row">
                      ...
                      {constructorData.companyUrl && (
                        <Button asChild variant="outline" className="w-full sm:w-auto">
                          <a href={constructorData.companyUrl} target="_blank" rel="noopener noreferrer">
            ...
            {constructorData.workScope && (
              <Card>
                ...
                <CardContent>
                  <p className="whitespace-pre-wrap text-muted-foreground">{constructorData.workScope}</p>
                </CardContent>
              </Card>
            )}
              ...
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between border-b pb-2">
                  ...
                  <span className="font-medium">
                    {constructorData.companyEstablishedAt ? new Date(constructorData.companyEstablishedAt).toLocaleDateString('ja-JP') : '—'}
                  </span>
                </div>
                <div className="flex items-center justify-between border-b pb-2">
                  <span className="text-muted-foreground">資本金</span>
                  <span className="font-medium">{constructorData.companyCapital ? `${constructorData.companyCapital.toLocaleString()}円` : '—'}</span>
                </div>
                <div className="flex items-center justify-between border-b pb-2">
                  <span className="text-muted-foreground">年間売上高</span>
                  <span className="font-medium">{constructorData.companyAnnualRevenue ? `${constructorData.companyAnnualRevenue.toLocaleString()}円` : '—'}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">加入保険</span>
                  <span className="font-medium">{constructorData.insuranceCoverage ?? '—'}</span>
                </div>
              </CardContent>
            </Card>

            {constructorData.availableAreas && constructorData.availableAreas.length > 0 && (
              <Card>
                <CardHeader>
